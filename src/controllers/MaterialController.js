...
router.route('/')
    .get((req, res) => {
        ...
        if (_id === "todos") Material.find().then(materiais => res.status(200).json({ sucesso: true, mensagem: "Materiais cadastradas no sistema.", quantidade: materiais.length, materiais }))
            ...
            Material.find(pesquisa).then(materiais => res.status(200).json({ sucesso: true, mensagem: "Materiais cadastradas no sistema.", quantidade: materiais.length, materiais }))
